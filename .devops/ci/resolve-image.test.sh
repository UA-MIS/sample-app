...
app: sample
environments:
  dev:     { trigger: "branch:main",   tagConvention: "sha-<short>", overlay: ".devops/chart/overlays/dev",     gate: auto }
...
# 1) push to main -> dev, mutable short-sha, PUSH (D-030 non-prod mutable tag)
run "main->dev" push "refs/heads/main" "9b08056abcdef1234567890"
assert_rc "main->dev" 0
assert_kv "main->dev" ENV dev
assert_kv "main->dev" TAG 9b08056
assert_kv "main->dev" PUSH true
assert_kv "main->dev" IMAGE "harbor.127-0-0-1.sslip.io/sample/sample:9b08056"
...
if PATH="${NOYQ_DIR}" command -v yq >/dev/null 2>&1; then
  ...
  assert_kv "noyq-main" IMAGE "harbor.127-0-0-1.sslip.io/sample/sample:9b08056"
